...
import com.rapdog.rapbot.api.TodayInHistoryApi;
 ...
 * @author rapdog
 */
public class TodayHistoryCommand extends BaseCommand {

    private static final Logger logger = LoggerFactory.getLogger(TodayHistoryCommand.class);

    public TodayHistoryCommand(MiraiGroupMessageEvent event){
        this.setEvent(event);
        // 指令配置
        this.setCommandString(CommandConstants.COV);
        this.setCommandDesc(CommandConstants.TODAY_HISTORY_CHINESE);
        // 截取指令参数
    ...
    public void doCommand() {
        logger.info("sending todayInHistory info");
        this.getEvent().sendBlocking(TodayInHistoryApi.getTodayInHistory());
    }
